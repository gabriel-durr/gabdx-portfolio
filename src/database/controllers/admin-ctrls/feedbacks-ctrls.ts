...
const getAllFeedbacks = async (res: NextApiResponse) => {
  try {
    const feedbackListOfPosts = await FeedbackModel.find({}).select('postId feedbackList')

    if (!feedbackListOfPosts) return res.status(404).json({ message: 'No feedback found on posts' })
  ...
  try {
    if (!idPost || !feedbackId)
      return res.status(400).json({
        message: 'It is necessary to inform the _id of the post and the feedback'
      })
