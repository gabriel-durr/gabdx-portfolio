...
const getAllFeedbacksSpam = async (res: NextApiResponse) => {
	try {
		...
		if (!feedbackList)
			return res.status(404).json({ message: "No feedback list found" });
...
const removeAllFeedbackSpam = async (
	req: NextApiRequest,
	res: NextApiResponse
) => {
	const { body } = req;
	...
	try {
		if (!idsFeedbackSpam.length)
			return res
				.status(400)
				.json({ messsage: "There is no feedbackId in the list" });

		const result = await FeedbackModel.updateMany(
			{ "feedbackList.feedbackId": { $in: idsFeedbackSpam } },
			{ $pull: { feedbackList: { feedbackId: { $in: idsFeedbackSpam } } } }
		);
