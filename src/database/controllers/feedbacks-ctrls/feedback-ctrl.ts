...
const getAllFeedbacks = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (!postCollection) return res.status(404).json({ message: 'PostId not found.' })

    const feedbackListOrigin = postCollection?.feedbackList

    if (!feedbackListOrigin) return res.status(404).json({ message: 'Feedbacks not found' })

    const totalFeedbacks = postCollection.feedbackList.map((f) => f.feedbackLevel).length
