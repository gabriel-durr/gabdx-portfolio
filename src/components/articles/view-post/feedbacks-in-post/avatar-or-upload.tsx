...
export const AvatarOrUpload = ({
  ...
  const [borderLight, borderDark] = useToken('colors', ['light.600', 'gray.300'])
          ...
          >
            {({ onImageUpload, dragProps }) => (
              <Box
                ...
                onClick={isAvatarExists ? () => handleRemoveAvatar() : () => onImageUpload()}
                _hover={{
                  background: '#020101f1',
                  fontWeight: 'medium'
                }}
              >
                <Icon fontSize="md" as={isAvatarExists ? TbPhotoX : TbPhotoEdit} />
              </Box>
            )}
          </ImageUploading>
        )}
