...
type LikeOrDislikeProps = Pick<
	FeedbackList,
	"feedbackId" | "likes" | "dislikes"
> & {
	lang: string;
};
...
export const LikeOrDislike = ({
	...
	async function handleLike(isLike: boolean, feedbackId: string) {
		try {
			...
			toast({
				title: isFeedbackRequired
					? likeLang.requireFeedbackMsg
					: likeLang.errorServer,
				status: "error",
			});
		}
	}
	...
	async function handleDislike(isDislike: boolean, feedbackId: string) {
		try {
			isDislike
				? await removeDislike(feedbackId)
				: await addDislike(feedbackId);
		} catch ({ response }: any) {
			const isFeedbackRequired = response.status === 400;

			toast({
				title: isFeedbackRequired
					? dislikeLang.requireFeedbackMsg
					: dislikeLang.errorServer,
				status: "error",
			});
		}
	}
