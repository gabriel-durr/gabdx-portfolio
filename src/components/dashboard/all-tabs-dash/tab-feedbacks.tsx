...
export const TabFeedbacksPainel = ({ feedbackListOfPosts }: TabFeedbacksPainelProps) => {
  return (
    <TabPanel>
      <AbsoluteCenter
        pos="absolute"
      ...
      >
        ...
        >
          {feedbackListOfPosts.map(({ _id, postId, feedbackList }) => (
            <Stack key={postId} w="full" align="center" spacing={14} mb={20} p={8}>
              ...
              >
                {feedbackList.map((feedback) => (
                  <VStack key={feedback._id.toString()} bg="whiteAlpha.100" p={4} rounded="sm">
                    <HStack spacing={8}>
                      <Avatar size="sm" name={feedback.name} src={feedback.avatar} />
                      <Text fontWeight="bold">
                        Name:
                        <Text as="span" fontWeight="normal" px={2} color="gbdx.sun">
                          {feedback.name}
                        </Text>
                      </Text>
                    ...
                    <Stack direction="row" w="full" px={4} justify="space-between">
                      <HStack>
                        <Text>
                          Likes:
                      ...
                      <DeleteFeedback idPost={_id.toString()} idFeedback={feedback.feedbackId} />
                    </Stack>
                  </VStack>
                ))}
              </VStack>
