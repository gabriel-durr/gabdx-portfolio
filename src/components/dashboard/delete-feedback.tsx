...
export const DeleteFeedback = ({ idPost, idFeedback }: DeleteFeedbackProps) => {
  ...
  async function handleDeleteSpam() {
    try {
      const {
        data: { message }
      } = await api.delete(`/admin/feedbacks?idPost=${idPost}&feedbackId=${idFeedback}`)
    ...
    <>
      ...
      >
        ...
        <AlertDialogContent bg="gbdx.white" fontFamily="body">
          <AlertDialogHeader>Do yout really want to delete this feedback?</AlertDialogHeader>
          <AlertDialogCloseButton />
          <AlertDialogBody>If accepted, feedback will be deleted from the post</AlertDialogBody>
          <AlertDialogFooter>
