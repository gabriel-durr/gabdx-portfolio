...
export const UnbanLocation = ({ idLocation }: CloseReportProps) => {
  ...
  async function handleRemoveBan() {
    try {
      ...
      toast({
        title: message,
        description: 'A server error occurred while trying to unban the location',
        status: 'error'
      })
    }
  }
    ...
    <>
      ...
      >
        ...
        <AlertDialogContent bg="gbdx.white" fontFamily="body">
          <AlertDialogHeader>Do you really want to unban this location?</AlertDialogHeader>
          <AlertDialogCloseButton />
          <AlertDialogBody>If accepted, that location will be unbanned</AlertDialogBody>
          <AlertDialogFooter>
