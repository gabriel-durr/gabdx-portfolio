...
export type CreateFeedbackType = Pick<FeedbackList, 'feedbackLevel' | 'name' | 'comment'>
...
export type UpdatedFeedbackType = Partial<Pick<FeedbackList, 'avatar' | 'comment'>>
...
const useFeedback = () => {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR<FeedbacksData, Error>(
    `/feedbacks?postId=${queryPostId}`,
    fetcher,
  ...
  async function reportThisUser({ reportedByName, reporterTo, typeOfReport }: ReportFeedbackType) {
    try {
